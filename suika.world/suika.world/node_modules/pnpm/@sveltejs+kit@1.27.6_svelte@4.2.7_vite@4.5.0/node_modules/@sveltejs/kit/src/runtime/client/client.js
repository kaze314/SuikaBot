...
export function create_client(app, target) {
  ...
  async function get_navigation_result_from_branch({
    ...
    const result = {
      ...
      props: {
        ...
        constructors: compact(branch).map(
    ...
    for (
      let i = 0;
      i < Math.max(branch.length, current.branch.length);
      i += 1
    ) {
      const node = branch[i];
  ...
  async function load_route({ id, invalidating, url, params, route }) {
    ...
    const url_changed = current.url
      ? id !== current.url.pathname + current.url.search
      : false;
    ...
    for (let i = 0; i < loaders.length; i += 1) {
      if (loaders[i]) {
        try {
          ...
          if (
            server_data_nodes?.includes(
              /** @type {import('types').ServerErrorNode} */ (err)
            )
          ) {
            // this is the server error rethrown above, reconstruct but don't invoke
            // the client error handler; it should've already been handled on the server
            status =
              /** @type {import('types').ServerErrorNode} */ (err).status ??
              status;
        ...
        try {
          return {
            ...
            node: {
              ...
              loader: /** @type {import('types').CSRPageNodeLoader } */ (
                errors[i]
              ),
              data: {},
              server: null,
  ...
  async function navigate({
    ...
    if (navigation_result.type === "redirect") {
      ...
      /** @type {number} */ (navigation_result.props.page?.status) >= 400
    ) {
      const updated = await stores.updated.check();
      if (updated) {
        await native_navigation(url);
      ...
      const after_navigate = (
        await Promise.all(
          callbacks.on_navigate.map((fn) =>
            fn(
              /** @type {import('@sveltejs/kit').OnNavigate} */ (nav.navigation)
            )
          )
        )
      ...
      (entries) => {
        for (const entry of entries) {
          if (entry.isIntersecting) {
            preload_code(
              get_url_path(
                new URL(/** @type {HTMLAnchorElement} */ (entry.target).href)
              )
            );
  ...
  return {
    ...
    _start_router: () => {
      ...
      container.addEventListener("submit", (event) => {
        ...
        const { keep_focus, noscroll, reload, replace_state } =
          get_router_options(event_form);
        if (reload) return;
    ...
    _hydrate: async ({
      ...
      if (!__SVELTEKIT_EMBEDDED__) {
        ...
        ({ params = {}, route = { id: null } } =
          get_navigation_intent(url, false) || {});
      }
...
async function load_data(url, invalid) {
  ...
  return new Promise(async (resolve) => {
    ...
    const reader = /** @type {ReadableStream<Uint8Array>} */ (
      res.body
    ).getReader();
    ...
    while (true) {
      ...
      while (true) {
        ...
        if (node.type === "data") {
          ...
          const deferred = /** @type {import('types').Deferred} */ (
            deferreds.get(id)
          );
          deferreds.delete(id);
